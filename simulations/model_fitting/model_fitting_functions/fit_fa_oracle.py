...
def run_simulations_fa_oracle(params_scenario, param, ds_seed):
    ...
    with open(os.path.join(os.path.join('results', 'fa_oracle'), file_name), 'wb') as file:
        dill.dump(model_final, file)

    reconstruction_loss_list = []
    for i in range(how_many_models):
        reconstruction_loss_list.append(model_list[i].fa.nodelist_y[0].elbo + model_list[i].fa.nodelist_y[1].elbo+model_list[i].fa.nodelist_y[2].elbo)
    ...
    with open(os.path.join(os.path.join('results', 'fa_oracle'), file_name2), 'wb') as file:
        dill.dump(model_final, file)
